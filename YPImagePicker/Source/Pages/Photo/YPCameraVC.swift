...
public class YPCameraVC: UIViewController, UIGestureRecognizerDelegate, YPPermissionCheckable {
    ...
    var photoCapture: YPPhotoCapture? = newPhotoCapture()
    var v: YPCameraView?
    internal var selection = [UIImage]()
    ...
    func shoot() {
        ...
        photoCapture!.shoot { imageData in
            ...
            //let img = YPMediaItem.photo(p: YPMediaPhoto(image: image.resetOrientation().resizedImageIfNeeded(), fromCamera: true))
            self.selection.append(image.resetOrientation().resizedImageIfNeeded())
        }
    }
    
    deinit {
        print("NS - CameraVC deinited 👍")
    }
    ...
    public func selectedMedia(multipleItemsCallback: @escaping (_ items: [YPMediaItem]) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for image in self.selection {
                resultMediaItems!.append(YPMediaItem.photo(p: YPMediaPhoto(image: image, fromCamera: true)))
            }
            
            DispatchQueue.main.async {
                multipleItemsCallback(resultMediaItems!)
                print("here123")
                resultMediaItems = nil
                self.v = nil
