...
public class YPPickerVC: YPBottomPager, YPBottomPagerDelegate {
    ...
    func done() {
        ...
        guard let cameraVC = cameraVC else { print("⚠️ YPPickerVC >>> YPCameraVC deallocated"); return }
        ...
        if mode == .library {
            ...
            cameraVC.selectedMedia(multipleItemsCallback: { [weak self] items in
                if items.count == 0 {
                    self?.close()
                } else {
                    self?.didSelectItems?(items)
